...
By default, Gozzi performs **full builds** that generate all pages from scratch. This ensures correctness and simplicity - no stale data or cache invalidation bugs.
...
Use `--clean` flag when you need a fresh build that removes ALL files first:
